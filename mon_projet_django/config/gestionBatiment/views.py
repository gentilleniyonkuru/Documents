    ...
    def inscription(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                logger.error(
                    f"Erreur d'envoi de l'email de bienvenue pour le client {client.id}: {str(e)}"
                )
            # ----------------------------------------------------
    ...
    def mon_profil(self, request):
        ...
        if profile is None:
            return Response(
                {
                    "has_profile": False,
                    "detail": "Aucun profil client trouvé pour cet utilisateur.",
                },
...
class NiveauViewSet(BaseModelViewSet):
    ...
    @action(
        detail=False, methods=["get"], url_path="par-batiment/(?P<batiment_id>[0-9]+)"
    )
...
class BureauViewSet(BaseModelViewSet):
    ...
    def get_queryset(self):
        ...
        if role in CLIENT_ROLES and profile is not None:
            bureaux_reserves = Bureau.objects.filter(
                reservations__client=profile, reservations__is_active=True
            )
...
class ReservationViewSet(BaseModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            if (
                reservation.client
                and reservation.client.user
                and reservation.client.user.email
            ):
                bureau_nom = (
                    reservation.bureau.code
                    if hasattr(reservation.bureau, "code")
                    else f"#{reservation.bureau.id}"
                )
                batiment_nom = (
                    reservation.bureau.niveau.batiment.nom
                    if hasattr(reservation.bureau, "niveau")
                    else "votre bâtiment"
                )
            ...
            logger.error(
                f"Erreur d'envoi de l'email pour la réservation {reservation.id}: {str(e)}"
            )
        # ------------------------------------------------
...
class ContratViewSet(BaseModelViewSet):
    ...
    def soumettre(self, request, pk=None):
        ...
        if role not in CLIENT_ROLES or profile is None or contrat.client != profile:
            return Response(
                {
                    "detail": "Seuls les clients propriétaires peuvent soumettre un contrat."
                },
        ...
        try:
            ...
            logger.error(
                f"Erreur notification soumission contrat {contrat.id}: {str(e)}"
            )
    ...
    def accepter(self, request, pk=None):
        ...
        if role != ADMIN_ROLE:
            return Response(
                {"detail": "Seuls les administrateurs peuvent accepter un contrat."},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def rejeter(self, request, pk=None):
        ...
        if role != ADMIN_ROLE:
            return Response(
                {"detail": "Seuls les administrateurs peuvent rejeter un contrat."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            if contrat.client and contrat.client.user and contrat.client.user.email:
                reason_text = (
                    contrat.rejection_reason
                    if contrat.rejection_reason
                    else "Aucun motif spécifié."
                )
                send_mail(
...
class PaiementViewSet(BaseModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if contrat_id:
            try:
                ...
                montant_auto = contrat.montant or getattr(
                    contrat.reservation, "montant_calcule", None
                )
            ...
            try:
                ...
                if role in CLIENT_ROLES and location.client != profile:
                    raise DRFValidationError(
                        {"location": "Cette location ne vous appartient pas."}
                    )
                ...
                raise DRFValidationError(
                    {"location": "Location introuvable ou inactive."}
                )
        else:
            raise DRFValidationError(
                {
                    "detail": "Un identifiant de contrat ou de location valide est requis."
                }
            )
        ...
        if montant_auto is None or float(montant_auto) <= 0:
            raise DRFValidationError(
                {
                    "montant": "Impossible de générer un montant automatique valide pour cette entité."
                }
            )
    ...
    def valider_paiement(self, request, pk=None):
        ...
        user = request.user
        if not hasattr(user, "client_profile") or user.client_profile.role not in [
            "ADMIN",
            "TRAVAILLEUR",
            "MANAGER",
        ]:
        ...
        try:
            ...
            logger.error(
                f"Échec de l'envoi de l'email pour le paiement {paiement.id}. Erreur : {str(e)}"
            )
